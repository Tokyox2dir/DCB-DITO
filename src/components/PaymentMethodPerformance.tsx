...
export function buildPaymentMethodPerformance(
  ...
  series.forEach((s) => {
    const name = s.payment_method || 'Unknown'
  ...
  return Array.from(map.entries())
    .map(([method, v]) => {
      ...
      return {
        ...
        pendingPct: total ? (v.pending / total) * 100 : 0,
        failedPct: total ? (v.failed / total) * 100 : 0,
      }
    })
    .filter((r) => r.total > 0)
...
export default function PaymentMethodPerformance({ series, loading, referenceNow }: Props) {
  const rows = useMemo(
    () => buildPaymentMethodPerformance(series, { now: referenceNow || undefined, limit: 20 }),
    [series, referenceNow],
  )

  return (
    <div className='mon-panel mon-panel-perf'>
      <div className='mon-perf-title'>Payment Method Performance</div>
      <div className='mon-perf-scroll'>
        ...
        {rows.map((r) => {
          ...
          return (
            <div className='mon-perf-item' key={r.method}>
              <div className='mon-perf-name'>
                <span className='mon-perf-client'>{r.method}</span>
